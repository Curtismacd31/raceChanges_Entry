...
//////////////////////////////////////////////////////////////////////////////////
//FTP HELPER FUNCTION
function fetchFtpFiles() {
  const ftpUrl = document.getElementById("ftpUrl").value;
  ...
  .then(data => {
    if (Array.isArray(data.files)) {
      const list = document.getElementById("ftpFileList");
      list.innerHTML = "";
  ...
  .catch(err => {
    ...
    alert("Failed to list FTP files.");
  });
}

function downloadSelectedFtpFile() {
  const file = document.getElementById("ftpFileList").value;
  const ftpUrl = document.getElementById("ftpUrl").value;
  const ftpUsername = document.getElementById("ftpUsername").value;
  const ftpPassword = document.getElementById("ftpPassword").value;
  ...
  .then(data => {
    if (data.success) {
      alert("✅ File downloaded and processed.");
      // Optional: auto-trigger load again
  ...
  .catch(err => {
    ...
    alert("Failed to download or extract file.");
  });
}
